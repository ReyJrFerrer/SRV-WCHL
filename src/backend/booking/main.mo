...
actor BookingCanister {
    ...
    type ProviderAnalytics = Types.ProviderAnalytics;
        ...
        switch (serviceCanisterId) {
            case (?serviceCanisterId) {
                let serviceCanister = actor(Principal.toText(serviceCanisterId)) : actor {
                    isServiceAvailable : (Text, Time.Time) -> async Result<Bool>;
                };
        ...
        switch (serviceCanisterId) {
            case (?serviceId) {
                let serviceCanister = actor(Principal.toText(serviceId)) : actor {
                    isProviderAvailable : (Principal, Time.Time) -> async Result<Bool>;
                };
    ...
    public shared(msg) func createBooking(
        ...
        switch (serviceCanisterId) {
            case (?serviceCanisterId) {
                let serviceCanister = actor(Principal.toText(serviceCanisterId)) : actor {
                    getService : (Text) -> async Result<Service>;
                    getPackage : (Text) -> async Result<ServicePackage>;
                };
    ...
    public func getServiceAvailableSlots(
        ...
        switch (serviceCanisterId) {
            case (?serviceCanisterId) {
                let serviceCanister = actor(Principal.toText(serviceCanisterId)) : actor {
                    getAvailableTimeSlots : (Text, Time.Time) -> async Result<[AvailableSlot]>;
                };
    ...
    public func getServiceAvailabilitySettings(serviceId : Text) : async Result<ProviderAvailability> {
        switch (serviceCanisterId) {
            case (?serviceCanisterId) {
                let serviceCanister = actor(Principal.toText(serviceCanisterId)) : actor {
                    getServiceAvailability : (Text) -> async Result<ProviderAvailability>;
                };
    ...
    public func getProviderAvailableSlots(
        ...
        switch (serviceCanisterId) {
            case (?serviceId) {
                let serviceCanister = actor(Principal.toText(serviceId)) : actor {
                    getAvailableTimeSlots : (Principal, Time.Time) -> async Result<[AvailableSlot]>;
                };
    ...
    public func getProviderAvailabilitySettings(providerId : Principal) : async Result<ProviderAvailability> {
        switch (serviceCanisterId) {
            case (?serviceId) {
                let serviceCanister = actor(Principal.toText(serviceId)) : actor {
                    getProviderAvailability : (Principal) -> async Result<ProviderAvailability>;
                };
    ...
    public shared(msg) func getProviderAnalytics(
        ...
        startDate : ?Time.Time,
        endDate : ?Time.Time
    ) : async Result<ProviderAnalytics> {
        let caller = msg.caller;
