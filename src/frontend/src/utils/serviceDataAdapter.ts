...
export function adaptServiceData(services: any[]) {
  return services.map(service => {
    ...
    if (service.heroImage) { // If heroImage comes directly from require()
        adaptedService.heroImage = convertImageToObjectOrPath(service.heroImage);
    } else if (service.media && service.media.length > 0 && service.media[0].url) { // If from media array
        adaptedService.heroImage = convertImageToObjectOrPath(service.media[0].url);
    } else {
    ...
    if (adaptedService.media) {
        adaptedService.media = service.media.map((item: any) => ({
            ...
            thumbnail: item.thumbnail ? convertImageToObjectOrPath(item.thumbnail) : undefined
        }));
    }
